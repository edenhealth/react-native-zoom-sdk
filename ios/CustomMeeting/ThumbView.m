...
- (void)showThumbView {
    ...
    if (!landscape) {
        [UIView animateWithDuration:0.25 animations:^{
            self.frame = CGRectMake(0, [UIScreen mainScreen].bounds.size.height-Bottom_Height-[self getCellSize]-24, [UIScreen mainScreen].bounds.size.width, [self getCellSize]+24);
...
- (void)hiddenThumbView {
    ...
    if (!landscape) {
        [UIView animateWithDuration:0.25 animations:^{
            float offset = IPHONE_X ? 24 : 0;
            self.frame = CGRectMake(0, [UIScreen mainScreen].bounds.size.height-[self getCellSize]-offset-24, [UIScreen mainScreen].bounds.size.width, [self getCellSize]+24);
        } completion:^(BOOL finished) {
        }];
    }
}
...
- (void)updateFrame
{
    ...
    {
        float offset = IPHONE_X ? 24 : 0;
        self.frame = CGRectMake(frame.size.width-[self getCellSize]-offset-24, 0, [self getCellSize]+24, frame.size.height);
    }
    else
    {
        self.frame = CGRectMake(0, frame.size.height-Bottom_Height-[self getCellSize]-24, frame.size.width, [self getCellSize]+24);
    }
    ...
    if (landscape)
    {
        ...
        self.thumbTableView.frame = CGRectMake(24, 0, [self getCellSize], frame.size.height);
        
        btnFrame = CGRectMake(0, 0, 24, self.frame.size.height);
        btnImageName = @"arrow_right_normal.png";
    }
    else
    {
        self.thumbTableView.transform = CGAffineTransformMakeRotation(-M_PI / 2);
        self.thumbTableView.frame = CGRectMake(0, 24, frame.size.width, [self getCellSize]);
        btnFrame = CGRectMake(0, 0, self.frame.size.width, 24);
...
- (void)onThumbHideClicked:(id)sender
{
    ...
    if (landscape) {
        if (self.thumbHidden) {
            btnFrame = CGRectMake([self getCellSize], 0, 24, self.frame.size.height);
        } else {
            btnFrame = CGRectMake(0, 0, 24, self.frame.size.height);
        }
        ...
        if (self.thumbHidden) {
            btnFrame = CGRectMake(0, [self getCellSize], self.frame.size.width, 24);
        } else {
            btnFrame = CGRectMake(0, 0, self.frame.size.width, 24);
        }
